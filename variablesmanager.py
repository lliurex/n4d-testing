...
class VariablesManager:
	...
	def set_attr(self,name,attr):
		...
		if name in self.variables:
			...
			return N4dLib.build_call_successful_response(True,"Attributes set")
		
		return N4dLib.build_call_failed_response(None,"Variable not found",VariablesManager.VARIABLE_NOT_FOUND_ERROR)
	...
	def delete_attr(self,name,key):
		...
		if name in self.variables:
			...
			return N4dLib.build_call_successful_response(True,"Attribute deleted")
	...
	def get_variable(self,name,full_description=False):
		...
		if name in self.variables:
			...
			if "root_protected" in self.variables[name] and self.variables[name]["root_protected"]:
				return N4dLib.build_call_failed_response(None,"Root protected variable. File is found in %s%s"%(VariablesManager.WATCH_DIR,name),VariablesManager.PROTECTED_VARIABLE_ERROR)
			
			if full_description:
				return N4dLib.build_call_successful_response(copy.deepcopy(self.variables[name]))
			else:
				return N4dLib.build_call_successful_response(copy.deepcopy(self.variables[name]["value"]))
			...
			if self.variables["REMOTE_VARIABLES_SERVER"]["value"] not in self.core.get_all_ips():
				...
				try:
					...
					return N4dLib.build_call_failed_response(None,str(e),VariablesManager.REMOTE_VARIABLES_SERVER_ERROR,tback)
	...
	def get_variables(self,full_info=False):
		...
		if full_info:
			return N4dLib.build_call_successful_response(copy.deepcopy(self.variables))
		...
		return N4dLib.build_call_successful_response(ret)
	...
	def delete_variable(self,name):
		...
		if name in self.variables:
			...
			return N4dLib.build_call_successful_response(True,"Variable deleted")
